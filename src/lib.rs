...
pub async fn back_up(aus: AuthorizedUserSecret, drive_folder_name: &str, bucket_name: &str) -> Result<()> {
    ...
    let mut files = vec![];
    loop {
        let file_list_response = list_files_in_folder(&hub, folder_id, &mut page_token).await.
            chain_err(|| format!("Could not list files in {drive_folder_name} folder in drive."))?.1;
...
async fn list_files_in_folder(hub: &DriveHub<HttpsConnector<HttpConnector>>,
                              folder_id: &String,
