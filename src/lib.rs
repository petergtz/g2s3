...
pub async fn back_up(aus: AuthorizedUserSecret) -> Result<()> {
    ...
    let files = hub.files().list().
        q("name = 'Takeout' and mimeType = 'application/vnd.google-apps.folder' and 'root' in \
        parents").
        param("fields", "files(id,name,parents,size)").doit().await.chain_err(|| "Could not find Takeout folder in drive.")?;
    assert_eq!(files.1.files.as_ref().unwrap().len(), 1);
    let takeout_folder_id = files.1.files.as_ref().unwrap().first().as_ref().unwrap().id.as_ref().unwrap();

    let mut page_token: Option<String> = None;

    let mut files = vec![];
    loop {
        let file_list_response = list_files_in_takeout_folder(&hub, takeout_folder_id, &mut page_token).await.chain_err(|| "Could not list files in Takeout folder in drive.")?.1;
    ...
    stream::iter(files).for_each_concurrent(
        4,
        |file| {
            let (hub, s3) = (hub.clone(), s3.clone());
            let tx = tx.clone();
            async move {
                let result = copy_file(&*hub, &*s3, file.clone()).await;
...
async fn list_files_in_takeout_folder(hub: &DriveHub<HttpsConnector<HttpConnector>>,
                                      takeout_folder_id: &String,
                                      page_token: &mut Option<String>)
                                      -> google_drive3::Result<(Response<Body>, FileList)> {
    let mut list_query = hub.files().list().
        q(format!("'{}' in parents", takeout_folder_id).as_str()).
...
async fn create_drive_hub(aus: AuthorizedUserSecret) ->
...
async fn copy_file(hub: &DriveHub<HttpsConnector<HttpConnector>>, s3: &Client, file: google_drive3::api::File) -> Result<()> {
    let filename = file.name.as_ref().unwrap();
    info!("Copying file {}", filename);
    let start_time = Instant::now();

    let file_content = hub.files()
        .get(file.id.as_ref().unwrap()).param("alt", "media")
        .doit()
        .await.chain_err(|| format!("Could not download file contents from drive for {}.", filename))?.0;
    assert!(file_content.status().is_success());
    let _s3_response = s3.put_object().bucket("pego-google-photos-backup")
        .key(filename)
    ...
    info!("Throughput for file {} of size {} B: {} B/s",
                filename,
